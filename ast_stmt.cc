...
void Program::Emit() 
{
    ...
    Decl *main = global_scope->Lookup("main"); 
    FnDecl *main_fn = dynamic_cast<FnDecl*>(main); 
    ...
    if (main_fn->GetReturnType() != Type::intType)
    {
        ReportError::NoMainFound();
        return; 
    }
...
void StmtBlock::Check(Hashtable<Decl*> everything, Hashtable<Decl*> *prev_scope, Hashtable< Hashtable<Decl*>* > *class_scopes, Hashtable< Hashtable<Decl*>* > *interface_scopes, bool printErrors, Hashtable<Decl*> *global_scope, LoopStmt *curr_loop, Type *curr_fn_return_type, ClassDecl *curr_class, Hashtable<Decl*> *curr_class_full_scope)  
{
    ...
    Iterator<Decl*> iterator = locals->GetIterator(); 
    Decl *i = iterator.GetNextValue(); 
    while (i != NULL)
    {
        cout << i << endl; 
        i = iterator.GetNextValue(); 
    }*/
    ...
    /*
    cout << "Everything scope" << endl; 
    Iterator<Decl*> iterator1 = everything.GetIterator(); 
    Decl *dl = iterator1.GetNextValue(); 
    while (dl != NULL)
    {
        cout << dl << endl; 
        dl = iterator1.GetNextValue(); 
    }*/
...
Location *ReturnStmt::Emit(CodeGenerator *code_gen, Hashtable< Hashtable<Decl*>* > *class_scopes, Hashtable< Hashtable<Decl*>* > *interface_scopes, Hashtable<Decl*> *global_scope, const char *break_label, FnDecl *curr_fn)
{
    if (expr != NULL)
    {
        Location *toReturn = expr->ReadValue(code_gen, class_scopes, interface_scopes, global_scope, curr_fn); 
